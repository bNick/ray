...
{
		$query = "UPDATE  d0y13_ray_cantry SET  name =  '".(JRequest::getVar('name'))."',
...
url1 = '".$_POST['url1']."'
 WHERE  id =".(JRequest::getVar('cid'))." LIMIT 1 ;";
  ...
  window.location.href = '".str_replace('&amp;', '&', JRoute::_('index.php?option=com_k2&view=categories'))."'
</script>";
}
elseif( JRequest::getVar('save'))
{
		...
		$query = " SELECT * FROM 0y13_ray_naspunkt WHERE alias='".$alias."'; ";		
		$db->setQuery($query);				
...
else
{
	...
	$query="SELECT 
					*
			FROM 
					d0y13_ray_cantry where id=".(JRequest::getInt('cid'));	
